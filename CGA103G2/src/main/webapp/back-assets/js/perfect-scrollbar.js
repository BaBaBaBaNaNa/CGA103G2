 ...
 */
(function(b, a) {
    typeof exports === "object" && typeof module !== "undefined" ? module.exports = a() : typeof define === "function" && define.amd ? define(a) : (b.PerfectScrollbar = a())
}(this, (function() {
    function p(R) {
    ...
    var P = function(S) {
        ...
        if (!S.settings.suppressScrollY && S.containerHeight + S.settings.scrollYMarginOffset < S.contentHeight) {
            ...
            S.scrollbarYTop = J(T * (S.railYHeight - S.scrollbarYHeight) / (S.contentHeight - S.containerHeight))
        } else {
    ...
    function O(R, S) {
        ...
        if (S.isScrollbarYUsingRight) {
            if (S.isRtl) {
                V.right = S.contentWidth - (S.negativeScrollAdjustment + R.scrollLeft) - S.scrollbarYRight - S.scrollbarYOuterWidth
            } else {
    ...
    var d = function(R) {
        R.event.bind(R.scrollbarY, "mousedown", function(S) {
            return S.stopPropagation()
        });
        R.event.bind(R.scrollbarYRail, "mousedown", function(T) {
            var U = T.pageY - window.pageYOffset - R.scrollbarYRail.getBoundingClientRect().top;
            var S = U > R.scrollbarYTop ? 1 : -1;
            R.element.scrollTop += S * R.containerHeight;
            P(R);
            T.stopPropagation()
        });
        ...
        R.event.bind(R.scrollbarXRail, "mousedown", function(T) {
            var U = T.pageX - window.pageXOffset - R.scrollbarXRail.getBoundingClientRect().left;
            var S = U > R.scrollbarXLeft ? 1 : -1;
            R.element.scrollLeft += S * R.containerWidth;
            P(R);
            T.stopPropagation()
        })
    };
    ...
    function c(U, Z) {
        ...
        function W() {
            F(U, ah);
            U[ac].classList.remove(e.state.clicking);
            U.event.unbind(U.ownerDocument, "mousemove", V)
        }
        U.event.bind(U[aa], "mousedown", function(ai) {
            ag = T[ae];
            af = ai[X];
            ad = (U[S] - U[R]) / (U[Y] - U[ab]);
            U.event.bind(U.ownerDocument, "mousemove", V);
            U.event.once(U.ownerDocument, "mouseup", W);
            U[ac].classList.add(e.state.clicking);
            ai.stopPropagation();
            ai.preventDefault()
        })
    }
    ...
    var Q = function(T) {
        ...
        function V(ae, Y, Z) {
            ...
            while (X && X !== R) {
                ...
                var ad = p(X);
                var ac = [ad.overflow, ad.overflowX, ad.overflowY].join("");
                if (ac.match(/(scroll|auto)/)) {
    ...
    var K = function(V) {
        ...
        function W(am, ag, ah) {
            ...
            while (af && af !== T) {
                ...
                var al = p(af);
                var ak = [al.overflow, al.overflowX, al.overflowY].join("");
                if (ak.match(/(scroll|auto)/)) {
                    ...
                    var ai = af.scrollLeft - af.clientWidth;
                    if (ai > 0) {
