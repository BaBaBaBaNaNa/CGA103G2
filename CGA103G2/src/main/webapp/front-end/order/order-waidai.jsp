			...
			<div class="container">
				<div class="row">
					...
					<table>
						...
						<%System.out.println("qwuiafefesfs"); %>
							<c:if test="${ordersVO.memID == memID}">
								<c:if test="${ordersVO.ordersType == 0}">
									<tr>
										...
										<td><c:if test="${ordersVO.ordersStatus == 0}">完成</c:if>
											<c:if test="${ordersVO.ordersStatus == 1}">未完成</c:if> <c:if
												test="${ordersVO.ordersStatus == 2}">退回</c:if></td>
										<td>${ordersVO.ordersBuildDate}</td>
										<td>${ordersVO.ordersMakeDate}</td>
										<td>
											<FORM METHOD="post"
												ACTION="<%=request.getContextPath()%>/back-end/orddetails/orddetails.do"
												style="margin-bottom: 0px;">
												<input type="submit" value="明細"> <input
													type="hidden" value="${ordersVO.ordersID}"
													name="orderDetailId"> <input type="hidden"
													name="action" value="getOne_For_Display2">
											</FORM>
										</td>
