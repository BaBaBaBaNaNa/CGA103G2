...
@WebServlet("/functions/FunctionsServlet.do")
public class FunctionsServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { 
			...
			try {
				functionID = Integer.valueOf(str);
			} catch (Exception e) {
			...
			FunctionsVO functionsVO = functionsSvc.getOneFunctions(functionID);
			if (functionsVO == null) {
				errorMsgs.add("查無資料");
			}
			// Send the use back to the form, if there were errors
...
//			}
			...
			/*************************** 2.開始新增資料 ***************************************/
			FunctionsService functionsSvc = new FunctionsService();
			functionsVO = functionsSvc.addFunctions(functionName);
