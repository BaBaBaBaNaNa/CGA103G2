...
//			empVO.setEmp_hiredate(LocalDateTime.parse(emp_hiredate));
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/employee/employee_add.jsp.jsp");
				failureView.forward(req, res);
				return;
			}
			...
			/*************************** 2.開始新增資料 ***************************************/
			EmpService empSvc = new EmpService();
			empVO = empSvc.addEmp(emp_name, emp_account, emp_password, Integer.parseInt(emp_permission), emp_phone, emp_address, emp_job, emp_hiredate);
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
			req.setAttribute("empVO", empVO); // 資料庫取出的empVO物件,存入req
			String url = "/back-end/employee/employee_edit.jsp";
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			/*************************** 2.開始修改資料 *****************************************/
			EmpService empSvc = new EmpService();
			empVO = empSvc.updateEmp(emp_id, emp_name, emp_account, emp_password, Integer.parseInt(emp_permission), emp_phone, emp_address, emp_job, emp_hiredate);
